...
const HomePageProductList = () => {
 ...
 const productCards = Object.values(HomePageProductListConfig).map(product => {
    ...
    switch (product.type) {
      case 'large':
        return <LargeProductCard key={product.title} {...product} productId={product.id} className={styles.productCard} onClick={() => handleProductClick(product.id)} />;
      case 'medium':
        return <MediumProductCard key={product.title} productId={product.id} className={styles.productCard} onClick={() => handleProductClick(product.id)} />;
      case 'small':
        return <SmallProductCard key={product.title} productId={product.id} className={styles.productCard} onClick={() => handleProductClick(product.id)} />;
      default:
        return null;
    }
 });
